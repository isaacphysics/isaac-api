...
*/
UPDATE logged_events SET event_details=concat('{"message": ', event_details::text, '}')::jsonb, event_details_type='java.util.LinkedHashMap' WHERE event_type='CONTACT_US_FORM_USED' AND event_details::TEXT NOT LIKE '{%';
UPDATE logged_events SET event_details=concat('{"gameboardId": ', event_details, '}')::jsonb, event_details_type='java.util.LinkedHashMap' WHERE event_type='ADD_BOARD_TO_PROFILE' AND event_details::TEXT NOT LIKE '{%';
