...
(defimpl DatomicLogger
  ...
  (logEvent [this session-id event-json]
     (if @rd/conn
         ...
         (let [logging-attributes (map #(name (first %)) (q '[:find ?i :where [?e :db/ident ?i]
					                                             [_ :db.install/attribute ?e]
					                                             [(namespace ?i) ?n]
					                                             [(= ?n "logging.event")]] (db @rd/conn)))
               
               matches (filter #(some #{(name %)} logging-attributes) 
	                            (keys event))
               
               values (for [m matches] {(keyword "logging.event" (name m)) (m event)})]
