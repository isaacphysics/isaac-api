 ...
 *
 */
public class IsaacSymbolicVariableValidator implements IValidator {
    private static final Logger log = LoggerFactory.getLogger(IsaacSymbolicVariableValidator.class);
    ...
    public IsaacSymbolicVariableValidator(final String hostname, final String port) {
    ...
    public QuestionValidationResponse validateQuestionResponse(final Question question, final Choice answer)
        ...
        if (!(question instanceof IsaacSymbolicVariableQuestion)) {
            throw new IllegalArgumentException(String.format(
                    "This validator only works with Isaac Symbolic Variable Questions... (%s is not variable)",
                    question.getId()));
        }

        if (!(answer instanceof Formula)) {
            throw new IllegalArgumentException(String.format(
                    "Expected Formula for IsaacSymbolicVariableQuestion: %s. Received (%s) ", question.getId(),
                    answer.getClass()));
        }

        IsaacSymbolicVariableQuestion symbolicVariableQuestion = (IsaacSymbolicVariableQuestion) question;
        ...
        if (null == symbolicVariableQuestion.getChoices() || symbolicVariableQuestion.getChoices().isEmpty()) {
        ...
        if (null == feedback) {
            ...
            for (Choice c : symbolicVariableQuestion.getChoices()) {

                // ... that are of the Formula type, ...
                if (!(c instanceof Formula)) {
                    log.error("Validator for questionId: " + symbolicVariableQuestion.getId()
                ...
                if (null == formulaChoice.getPythonExpression() || formulaChoice.getPythonExpression().isEmpty()) {
                    log.error("Expected python expression, but none found in choice for question id: "
                            + symbolicVariableQuestion.getId());
                    continue;
                }
        ...
        if (null == feedback) {
            ...
            List<Choice> orderedChoices = getOrderedChoices(symbolicVariableQuestion.getChoices());
                    ...
                    if (response.containsKey("error")) {
                        if (response.containsKey("code")) {
                            ...
                            log.warn("Problem checking formula \"" + submittedFormula.getPythonExpression()
                                    + "\" for (" + symbolicVariableQuestion.getId() + ") with symbolic checker: " + response.get("error"));
                        }
                    } else {
                ...
                if (closestMatchType != MatchType.EXACT && closestMatch.getRequiresExactMatch()) {
                    if (closestMatch.isCorrect()) {
                        ...
                        log.info("User submitted an answer that was close to an exact match, but not exact "
                                + "for question " + symbolicVariableQuestion.getId() + ". Choice: "
        ...
        return new FormulaValidationResponse(symbolicVariableQuestion.getId(), answer, feedback, responseCorrect, responseMatchType.toString(), new Date());
    }
