...
public class QuizFacade extends AbstractIsaacFacade {
    ...
    public final Response getQuizAssignmentCSV(@Context final HttpServletRequest httpServletRequest,
        ...
        try {
            ...
            getQuizQuestionTitlesAndIds(questionTitles, questionIds, quiz);
    ...
    public final Response getQuizResultsForGroup(@Context final HttpServletRequest httpServletRequest,
        ...
        try {
            ...
            Map<String, Map<Long, QuizFeedbackDTO.Mark>> quizTotals = new HashMap<>();
...
//                    group.getGroupName(), new Date(), user.getGivenName(), user.getFamilyName()));
            ...
            for (QuizAssignmentDTO assignment : quizAssignments) {
                String quizId = assignment.getQuizId();
                IsaacQuizDTO quiz = this.quizManager.findQuiz(quizId);
                quizTotals.putIfAbsent(quizId, new HashMap<>());

                List<QuizUserFeedbackDTO> usersFeedback = getUserFeedback(user, assignment, quiz, groupMembers);
                Map<UserSummaryDTO, QuizFeedbackDTO> userFeedbackMap = usersFeedback.stream()
                        .filter(f -> f.getFeedback() != null)
                        .collect(Collectors.toMap(QuizUserFeedbackDTO::getUser, QuizUserFeedbackDTO::getFeedback));

                for (QuizUserFeedbackDTO f : usersFeedback) {
                    UserSummaryDTO groupMember = f.getUser();
                    QuizFeedbackDTO feedback = f.getFeedback();

                    if (!this.associationManager.hasPermission(user, groupMember)) {
                        continue;
                    }
                    if (feedback == null) {
                        continue;
                    ...
                    quizTotals.get(quizId).put(groupMember.getId(), feedback.getOverallMark());

                    Map<String, QuizFeedbackDTO.Mark> questionMarks = feedback.getQuestionMarks();
                }
            }

            for (RegisteredUserDTO m : groupMembers) {
                UserSummaryDTO groupMember = this.userManager.convertToUserSummaryObject(m);
                List<String> row = new ArrayList<>(Arrays.asList(groupMember.getGivenName(), groupMember.getFamilyName()));

                if (!this.associationManager.hasPermission(user, groupMember)) {
                    row.add("PERMISSION REVOKED");
                    rows.add(row);
                    continue;
                }
                for (QuizAssignmentDTO assignment : quizAssignments) {
                    ...
                    QuizFeedbackDTO.Mark quizMark = quizTotals.get(quizId).get(groupMember.getId());
                    int quizPercentage = quizMark == null ? 0 : 100 * quizMark.correct / (quizMark.correct + quizMark.incorrect + quizMark.notAttempted);
                    row.add(String.format("%d", quizPercentage));
...
//                    .header("Content-Disposition", "attachment; filename=quiz_results.csv")
    ...
    private void getQuizQuestionTitlesAndIds(List<String> questionTitles, List<String> questionIds, IsaacQuizDTO quiz) {
