...
public class MongoUserDataManager implements IUserDataManager, IUserQuestionManager {
    ...
    public void deleteUserAccount(final RegisteredUser user) throws SegueDatabaseException {
        ...
        try {
            ...
            if (r.getError() != null) {
                log.error("Error during database update " + r.getError());
                throw new SegueDatabaseException("MongoDB encountered an exception while deleting a user account: "
                        + r.getError());
            }
    ...
    public RegisteredUser getByLegacyId(final String id) throws SegueDatabaseException {
        ...
        JacksonDBCollection<RegisteredUser, String> jc = JacksonDBCollection.wrap(
                database.getDB().getCollection(USER_COLLECTION_NAME), RegisteredUser.class, String.class, objectMapper);
        try {
    ...
    public void updateUserLastSeen(final RegisteredUser user, final Date date) throws SegueDatabaseException {
        ...
        JacksonDBCollection<BasicDBObject, String> jc = JacksonDBCollection.wrap(
                database.getDB().getCollection(USER_COLLECTION_NAME), BasicDBObject.class, String.class, objectMapper);

        Query q = DBQuery.is("_id", new ObjectId(userId));

        BasicDBObject update = new BasicDBObject("$set", new BasicDBObject(Constants.USER_LAST_SEEN_FIELDNAME, date));
    ...
    private void initialiseDataManager() {
        ...
        conn.getCollection(USER_COLLECTION_NAME).ensureIndex(
                new BasicDBObject(Constants.LOCAL_AUTH_EMAIL_FIELDNAME, 1), Constants.LOCAL_AUTH_EMAIL_FIELDNAME, true);
        ...
        conn.getCollection(LINKED_ACCOUNT_COLLECTION_NAME).ensureIndex(linkedAccountIndex, "LinkedAccountIndex", true);
        initialised = true;
    }

    /**
     * This helper method will attempt to remove all linked accounts for a given user id. It is expected that this will
     * only be used when a user is deleted.
     * 
     * @param userId
    ...
    private void cleanupOrphanedLinkedAccounts(final String userId) {
        ...
        DBQuery.Query linkAccountToDeleteQuery = DBQuery.is(Constants.LINKED_ACCOUNT_LOCAL_USER_ID_FIELDNAME, userId);
