...
public interface LocationHistory {
	 ...
	 * 
	 * You should check to see if an existing record exists and run
	 * updateLocationEventDate on it before creating a new one.
	 * 
	 * @param ipAddress of interest
	 * @param location (geo-coded)
	 * @return the newly created locationEvent Record.
	 * @throws JsonProcessingException if we cannot convert the location into something useful.
	 * @throws SegueDatabaseException - if there is a db error.
	 */
	 ...
	 * 
	 * @param id the database id for the record of interest
	 * @param isCurrent - whether or not this record should be treated as the current one.
	 * @throws SegueDatabaseException - if there is a db error.
	 */
