...
public class GameboardPersistenceManager {
	...
	public final String saveGameboardToPermanentStorage(final GameboardDTO gameboard) {
		uk.ac.cam.cl.dtg.isaac.dos.GameboardDO gameboardToSave = 
				mapper.map(gameboard, uk.ac.cam.cl.dtg.isaac.dos.GameboardDO.class);
	...
	public final List<GameboardDTO> getGameboardsByUserId(final String userId) {
		...
		fieldsToMatch.put(com.google.common.collect.Maps.immutableEntry(
				Constants.BooleanOperator.AND, USER_ID_FKEY), Arrays.asList(userId));
		
		List<GameboardDO> gameboardsFromDb 
			= this.gameboardDataManager.find(fieldsToMatch);
		
		List<GameboardDTO> gameboardDTOs = this.convertToGameboardDTOs(gameboardsFromDb);
	...
	private void tidyTemporaryGameboardStorage() {
		if (this.gameboardNonPersistentStorage.size() >= CACHE_TARGET_SIZE) {
			...
			for (GameboardDTO board : this.gameboardNonPersistentStorage.values()) {
	...
	private GameboardDTO convertToGameboardDTO(final GameboardDO gameboardDO) {		
		...
		ResultsWrapper<Content> results = api.findMatchingContent(
				api.getLiveVersion(), fieldsToMap, 0, gameboardDO.getQuestions().size());
