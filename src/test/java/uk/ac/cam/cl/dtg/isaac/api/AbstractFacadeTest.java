...
abstract public class AbstractFacadeTest extends IsaacTest {
  ...
  private void assertErrorResponse(Status expectedStatus, Response actual) {
    assertEquals("Expected status " + expectedStatus.name() + ", got " + actual.getStatusInfo().toEnum().name()
  ...
  class Testcase {
    ...
    private List<Step> checkSteps(Step[] steps) {
      ...
      List<Object> brokenMocks =
          mocksPrepared.entrySet().stream().filter(entry -> entry.getValue() > 1).map(entry -> entry.getKey())
              .collect(Collectors.toList());
      if (!brokenMocks.isEmpty()) {
        throw new IllegalArgumentException(
            "Test prepartion steps are broken. The following mocks are prepared multiple times:\r\n" + Joiner.on("\r\n")
