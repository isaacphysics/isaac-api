...
public class InfoFacadeTest extends IsaacTest {
    ...
    public void setUp() throws RuntimeException, IOException {
        PropertiesLoader properties = new PropertiesLoader("../isaac-other-resources/segue-test-config.properties");
        properties.setProperty("POSTGRES_DB_URL", "jdbc:postgresql://localhost:" + postgres.getMappedPort(5432) + "/rutherford");
        // Alternative via mocking:
        /*
        properties = createMock(PropertiesLoader.class);
        expect(properties.getProperty("SEARCH_CLUSTER_NAME")).andReturn("isaac").anyTimes();
        expect(properties.getProperty("SEARCH_CLUSTER_ADDRESS")).andReturn("localhost").anyTimes();
        expect(properties.getProperty("SEARCH_CLUSTER_PORT")).andReturn("9300").anyTimes();
        expect(properties.getProperty("SEARCH_CLUSTER_INFO_PORT")).andReturn("9200").anyTimes();
        expect(properties.getProperty("SEARCH_RESULTS_HARD_LIMIT")).andReturn("2000").anyTimes();
        expect(properties.getProperty("HOST_NAME")).andReturn("localhost:8080/isaac-api").anyTimes();
        expect(properties.getProperty("PROXY_PATH")).andReturn("/isaac-api").anyTimes();
        expect(properties.getProperty("MAILER_SMTP_SERVER")).andReturn("ppsw.cam.ac.uk").anyTimes();
        expect(properties.getProperty("MAIL_FROM_ADDRESS")).andReturn("cl-isaac-contact@lists.cam.ac.uk").anyTimes();
        expect(properties.getProperty("MAIL_RECEIVERS")).andReturn("af599@cam.ac.uk").anyTimes();
        expect(properties.getProperty("MAIL_NAME")).andReturn("Isaac Physics").anyTimes();
        expect(properties.getProperty("REPLY_TO_ADDRESS")).andReturn("support@isaacphysics.org").anyTimes();
        expect(properties.getProperty("SERVER_ADMIN_ADDRESS")).andReturn("af599@cam.ac.uk").anyTimes();
        expect(properties.getProperty("EMAIL_SIGNATURE")).andReturn("Isaac Physics Project").anyTimes();
        expect(properties.getProperty("EVENT_ADMIN_EMAIL")).andReturn("events@isaacphysics.org").anyTimes();
        expect(properties.getProperty("EVENT_ICAL_UID_DOMAIN")).andReturn("isaacphysics.org").anyTimes();
        expect(properties.getProperty("EVENT_GROUP_RESERVATION_LIMIT")).andReturn("69").anyTimes();
        // ...
        replay(properties);
         */
